...
import 'package:dartzmqcustom/src/bindings.dart';
...
class ZSocket {
  ...
  late final StreamController<ZMessage> controller;

  /// Stream of received [ZMessage]s
  Stream<ZMessage> get messages => controller.stream;

  /// Stream of received [ZFrame]s (expanded [messages] stream)
  Stream<ZFrame> get frames => messages.expand((element) => element.frames);

  /// Stream of received payloads (expanded [frames] strem)
  Stream<Uint8List> get payloads => frames.map((e) => e.payload);

  /// Construct a new [ZSocket] with a given underlying ZMQSocket [socket] and the global ZContext [_context]
  ZSocket(this.socket, this._context) {
    controller = StreamController(onListen: () {
      _context.listen(this);
    }, onCancel: () {
      _context.stopListening(this);
    });
  }
  ...
  void send(final List<int> data,
    ...
    final result =
        bindings.zmq_send(socket, ptr.cast(), data.length, sendParams);
    malloc.free(ptr);
    checkReturnCode(result, ignore: [EINTR]);
  }
  ...
  void bind(final String address) {
    ...
    final result = bindings.zmq_bind(socket, endpointPointer);
  ...
  void connect(final String address) {
    _checkNotClosed();
    final endpointPointer = address.toNativeUtf8();
    final result = bindings.zmq_connect(socket, endpointPointer);
    malloc.free(endpointPointer);
    checkReturnCode(result, ignore: [EINTR]);
  }
  ...
  void close() {
    if (!_closed) {
      _context.handleSocketClosed(this);
      bindings.zmq_close(socket);
      controller.close();
      _closed = true;
    }
  }
  ...
  void setOption(final int option, final String value) {
    ...
    final result =
        bindings.zmq_setsockopt(socket, option, ptr.cast<Uint8>(), ptr.length);
    malloc.free(ptr);
    checkReturnCode(result, ignore: [EINTR]);
  }
...
class MonitoredZSocket extends ZSocket {
  ...
  /// Construct a new [MonitoredZSocket] with a given underlying ZMQSocket [socket], the global ZContext [_context] and the given [event]s to monitor
