...
// Native bindings
final ZMQBindings bindings = ZMQBindings();
...
class ZContext {
  ...
  ZContext() {
    _context = bindings.zmq_ctx_new();
    _poller = bindings.zmq_poller_new();
    _startPolling();
  }
  ...
  void _poll() {
    ...
    final availableEventCount =
        bindings.zmq_poller_wait_all(_poller, pollerEvents, socketCount, 0);

    if (availableEventCount > 0) {
      final frame = ZMQBindings.allocateMessage();
      var rc = bindings.zmq_msg_init(frame); // rc == 0
      checkReturnCode(rc);
      ...
      for (var eventIdx = 0; eventIdx < availableEventCount; ++eventIdx) {
        ...
        while ((rc =
                bindings.zmq_msg_recv(frame, socket.socket, ZMQ_DONTWAIT)) >=
            0) {
          // final size = _bindings.zmq_msg_size(msg);
          final data = bindings.zmq_msg_data(frame).cast<Uint8>();
          final copyOfData = Uint8List.fromList(data.asTypedList(rc));

          final hasMore = bindings.zmq_msg_more(frame) != 0;

          zMessage.add(ZFrame(copyOfData, hasMore: hasMore));

          if (!hasMore) {
            socket.controller.add(zMessage);
            zMessage = ZMessage();
          }
        }

        checkReturnCode(rc, ignore: [EAGAIN, EINTR]);
      }

      rc = bindings.zmq_msg_close(frame); // rc == 0
      malloc.free(frame);
      checkReturnCode(rc);
    }
  ...
  bool hasCapability(final String capability) {
    ...
    final result = bindings.zmq_has(ptr);
    malloc.free(ptr);
    return result == 1;
  }
  ...
  ZSocket createSocket(final SocketType mode) {
    final socket = bindings.zmq_socket(_context, mode.index);
  ...
  void listen(ZSocket socket) {
    bindings.zmq_poller_add(_poller, socket.socket, nullptr, ZMQ_POLLIN);
    _listening.add(socket);
    _startPolling();
  }

  void stopListening(ZSocket socket) {
    bindings.zmq_poller_remove(_poller, socket.socket);
    _listening.remove(socket);
  }

  void handleSocketClosed(ZSocket socket) {
    if (!_shutdown) {
      _createdSockets.remove(socket.socket);
    }
